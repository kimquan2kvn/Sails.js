...
module.exports = {
  ...
  addClass: async function(req,res) {
    ...
    let classid = await Class.findOne({name: classroom});
    let Idclass = classid._id; 
    Student.validParams(studenId);
    // classroomAdd = req.body.classroom;
    
    await Student.addToCollection(studenId, 'classroom').members(Idclass);
