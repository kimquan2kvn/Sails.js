...
module.exports = {
  ...
  addClass: async function(req,res) {
    ...
    let classid = aawClass.findOne({name: classroom1});
    Student.find({id:studenId}).populate('classlists').then(user=>{
        // user[0].classlists.push(classid.id);
        res.json(user[0].classlists.push(classid));
    })
  }
};
