...
module.exports={
  ...
  register:function(req,res){
    ...
    User.create({firstName:firstName,lastName:lastName,email:email,password:password}).exec((err, user) => {
      if (err) {
        res.json(err.status, {err: err});
        return;
      }
      if (user) {
        res.json({ success: true, message: 'Registered successfully. Please, go ahead and login' });
      }
    });
  }
