...
module.exports={
  register:async function(req,res){
    ...
    if(!req.body.firstName){
      errors.push({message: 'Hãy điền firstName'});
    }
    if(!req.body.lastName){
        errors.push({message: 'Hãy điền lastName'});
    }
    if(!req.body.email){
        errors.push({message: 'Hãy điền email'});
    }
    if(!req.body.password){
        errors.push({message: 'Hãy điền password'});
    }
    if(!req.body.passwordConfirm){
        errors.push({message: 'Không được bỏ trống'});
    }
    try{
